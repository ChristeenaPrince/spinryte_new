...
console.log (productId)
  ...
  const handleEditClick = async (id) => {
    try {
      //const response = await axios.get(`https://spinryte.in/draw/api/Product/get_productDetails?id=${id}`);
      ...
      if (response.data && response.data.status) {
        const { name, description, price, created_at, status, category_id, product_images, attributeValues } = response.data.data;
        ...
        formik.setValues({
          ...
          productImage: product_images.map(image => ({ id: image.id, url: image.image, file: null })),
        ...
        // Check if product has images, if not, prompt to add
  ...
  /*
  const removeImage = async (image, id, index) => {
    try {
      if (id) {
      ...
      const newImages = formik.values.productImage.filter((img, imgIndex) => imgIndex !== index);
      formik.setFieldValue('productImage', newImages);
    } catch (error) {
      console.error('Error removing image:', error);
      showMessage('Error removing image.');
    }
  };
  ...
  onClick={() => {
    formik.validateForm().then((errors) => {
      if (Object.keys(errors).length === 0) {
        ...
        // If editing and no images exist, ask user to add images
        if (editItemId && (!formik.values.productImage || formik.values.productImage.length === 0)) {
          setOpenAddImageDialog(true);
        }
      }
    });
